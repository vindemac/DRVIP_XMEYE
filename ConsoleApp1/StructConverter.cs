...
public class StructConverter
{
    ...
    {
        if (o is char) return BitConverter.GetBytes((char)o);
        ...
        if (o is String) return Encoding.Default.GetBytes(o.ToString());
    ...
    public static byte[] Pack(object[] items, bool LittleEndian, out string NeededFormatStringToRecover)
    {
       ...
       /* for (int i = 0; i < items.Length; i++) {
            if (i == 0) {
                byte[] bitt = { byte.MinValue };
                outputBytes.AddRange(bitt);
            }
            if (i == 5)
            {
                byte[] bitt = { 16 };
            ...
            byte[] theseBytes = TypeAgnosticGetBytes(items[i]);
